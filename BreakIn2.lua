...
if game.PlaceId ~= 13864667823 then
	...
	Tab:AddDropdown({
		...
		Callback = function(Value)
			if Value == 'Book' or Value == 'Phone' then
				Notify('Warning', Value .. " Wont Work Unless You Own The Corresponding Gamepass.", 'rbxassetid://', 7)
			end
			SelectedItem = Value
		end
	})
	...
	Tab:AddToggle({
		...
		Default = false,
		Callback = function(Value)
			getgenv().LoopGiveItem = Value
			while LoopGiveItem do
				GiveItem(SelectedItem)
				task.wait(.1)
			end
		end
	...
	Tab:AddToggle({
		Name = "Loop Train Strength",
		Default = false,
		Callback = function(Value)
			getgenv().LoopTrainStrength = Value
			while LoopTrainStrength do
				Train("Strength")
				task.wait(.1)
			end
		end
	...
	Tab:AddToggle({
		Name = "Loop Train Speed",
		Default = false,
		Callback = function(Value)
			getgenv().LoopTrainSpeed = Value
			while LoopTrainSpeed do
				Train("Speed")
				task.wait(.1)
			end
		end
	...
	Tab:AddToggle({
		...
		Callback = function(Value)
			...
			if RemoveSlipping == true then
				Notify('Credits To', "Leo Dicap On V3rmillion For Making This Feature!", 'rbxassetid://', 7)
			end
		end
	})
	Tab:AddToggle({
	...
	Tab:AddButton({
		...
		Callback = function()
			if SemiGodmode == false then
				...
				Notify('Warning', "Damanging Yourself Won't Work Due To Having 'Semi Godmode' Enabled!", 'rbxassetid://', 7)
			end
		end
	})
	Tab:AddToggle({
		Name = "Loop Damange Yourself",
		Default = false,
		Callback = function(Value)
			getgenv().LoopDamangeYourself = Value
			while LoopDamangeYourself do
				TakeDamange(Damange)
				task.wait(1)
			end
		end
	...
	Tab:AddButton({
		...
		Callback = function()
			if RemoveSlipping == false then
				...
				Notify('Warning', "Slipping Won't Work Due To Having 'Remove Slipping' Enabled!", 'rbxassetid://', 7)
			end
		end
	})
	Tab:AddToggle({
	...
	Tab:AddButton({
		...
		Callback = function()
			if game:GetService("Workspace"):FindFirstChild("Villainess") then
				...
				Notify('Warning', "Scary Marry Is Already Deleted Or Boss Fight Hasnt Started!", 'rbxassetid://', 7)
			end
		end
	})
	...
	Tab:AddButton({
		...
		Callback = function()
			if game:GetService("Workspace"):FindFirstChild("BigBoss") then
				...
				Notify('Warning', "Scary Larry Is Already Deleted Or Boss Fight Hasnt Started!", 'rbxassetid://', 7)
			end
		end
	})
		...
		Callback = function(Value)
			if Value == true then
				for i, v in pairs(game:GetService("Workspace").Hidden:GetChildren()) do
					...
					highlight.Parent = v
					highlight.FillColor = Color3.fromRGB(235, 232, 232)
					highlight.OutlineColor = Color3.fromRGB(247, 245, 245)
				end
			else
				for i, v in pairs(game:GetService("Workspace").Hidden:GetChildren()) do
